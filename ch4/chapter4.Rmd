...
In the lognormal model overdispersion was accounted for in the linear predictor via the inclusion of the $\epsilon$ term (and its hyperparameter variance $\sigma$). In this negative-binomial formulation the amount of overdispersion is accounted for by the $\phi$ parameter. Because $\phi$ can take values both above and below one, the negative binomial distribution is capable of accounting for both over- and (more rarely encoutnered) underdispersion (when the variance is actually less than expected given the mean). Thus the negative-binomial is a bit more flexible than the lognormal when it comes to correctly modeling over-and and underdispersed data (though the lognormal has its own advantages).
