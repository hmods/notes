...
As we shall see, it ultimately makes no difference whether the covariates are continuous, categorical, or ordinal. 
...
To give you a little help, imagine that instead of having just the measurement for your focal individual, you have measurements for a larger sample of $n$ individuals.
Now, it might make sense that your best guess for any given individual will be the average of measurement for all the individuals in the population.
...
If its reasonable to think that $Y$ is normally distributed, then we can assume that the realizations or samples $y$ that we observe are also normally distributed: $y \sim N(\mu, \sigma^2)$. 
...
To assess this possibility, consider the *bivariate* confidence ellipse for these two parameters. 
...
This is not great, because the possible values for each parameter are heavily cconstrained by the estimate for the other, and we cannot take our univarite confidence intervals at face value.
...
Now for future work in make guinea pig teeth grow, we may want to know whether the dose and method of supplementation interact in their influence on length.
...
An alternative approach would be to use information theoretical to decide whether the interaction is warranted.
...
Here we see that the original model `m` with interaction has a lower AIC value, and is therefore better supported. Note that AIC is not without its own drawbacks (just how much lower AIC is really *lower*?).
...
```{r, echo = FALSE, fig.cap="Raw data along with confidence intervals for the means of each group when the interaction is assumed to be non-existent"}
g <- expand.grid(supp = levels(ToothGrowth$supp), 
                 dose = levels(ToothGrowth$dose))
p2 <- predict(m2, g, interval = 'confidence', type='response')
predictions2 <- cbind(g, data.frame(p2))
ggplot(ToothGrowth, aes(x=interaction(dose, supp), y=len)) + 
  geom_segment(data=predictions, 
               aes(y=lwr, yend=upr, 
                   xend=interaction(dose, supp)), col='red') + 
geom_segment(data=predictions2, 
               aes(y=lwr, yend=upr, 
                   xend=interaction(dose, supp)), col='blue') + 
  geom_point(data=predictions, aes(y=fit), color='red', size=2, shape=2) +
  geom_point(data=predictions2, aes(y=fit), color='blue', size=2, shape=2) +  
  geom_jitter(position = position_jitter(width=.1), shape=1) + 
  ylab("Length")
```
